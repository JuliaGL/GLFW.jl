...
GetVersionString() = Compat.String(ccall( (:glfwGetVersionString, lib), Cstring, ()))

# Error handling
@callback Error(code::Cint, description::Cstring) -> (code, Compat.String(description))
...
GetMonitorName(monitor::Monitor) = Compat.String(ccall( (:glfwGetMonitorName, lib), Cstring, (Monitor,), monitor))
...
@callback Drop(window::Window, count::Cint, paths::Ptr{Ptr{Cchar}}) -> (window, map(Compat.String, pointer_to_array(paths, count)))
...
GetJoystickName(joy::Integer) = Compat.String(ccall( (:glfwGetJoystickName, lib), Cstring, (Cint,), joy))
